...
extern IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hWindow, UINT message, WPARAM wideParm, LPARAM longParm);

LRESULT CALLBACK WindowProcess(HWND hWindow, UINT message, WPARAM wideParm, LPARAM longParm);
...
bool gui::SetupWindow(const char* name)
{
	window = CreateWindow(windowClass.lpszClassName, name, WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, 0, 0, windowClass.hInstance, 0);
...
void gui::Setup()
{
	...
	if (!SetupDirectX)
			throw std::runtime_error("Failed to d3d");
...
void gui::Render()
{
	...
	ImGui::Begin("Cool menu", &isVisible);
	ImGui::End();
...
LRESULT CALLBACK WindowProcess(HWND hWindow, UINT message, WPARAM wideParm, LPARAM longParm)
{
	...
	if (gui::isVisible && ImGui_ImplWin32_WndProcHandler(hWindow, message, wideParm, longParm)) return 1L;

	return CallWindowProc(gui::originalWindowProcess,hWindow,message,wideParm,longParm);
}